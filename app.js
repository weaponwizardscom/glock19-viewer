...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    await loadSvg(); buildUI(); defaultBlack();
    changeBackground(); updateText(); updateSummary();
    ...
    function changeBackground(){
        ...
        currentBg=next; gunView.style.background=`url("${next}") center/auto no-repeat`;
    }
    ...
    function randomize(){const keys=Object.keys(COLORS);PARTS.forEach(p=>{const pick=keys[Math.floor(Math.random()*keys.length)];applyColor(p.id,COLORS[pick],pick);});}
    ...
    async function savePng(){
        ...
        const BG_W=1600, BG_H=1200;            /* rzeczywiste wymiary tła */
        const canvas=Object.assign(document.createElement('canvas'),{width:BG_W+PANEL_W,height:BG_H});
        const ctx=canvas.getContext('2d');

        /* 1) tło */
        if(currentBg){
            const bg=await loadImg(currentBg); ctx.drawImage(bg,0,0,BG_W,BG_H);
        } else { ctx.fillStyle='#000'; ctx.fillRect(0,0,BG_W,BG_H); }

        /* centrowanie pistoletu */
        const PX_OFFSET_X = (BG_W - pistolW)/2;
        const PX_OFFSET_Y = (BG_H - pistolH)/2;

        /* 2) tekstura */
        const base=await loadImg(TEXTURE);
        ctx.drawImage(base,PX_OFFSET_X,PX_OFFSET_Y,pistolW,pistolH);

        /* 3) overlaye */
        await Promise.all([...svg.querySelectorAll('.color-overlay')].filter(o=>o.style.fill&&o.style.fill!=='transparent').map(ov=>{
            ...
            return loadImg(url).then(img=>{ctx.drawImage(img,PX_OFFSET_X,PX_OFFSET_Y,pistolW,pistolH);URL.revokeObjectURL(url);});
        }));

        /* 4) panel */
        const x0=BG_W; ctx.fillStyle='#000c'; ctx.fillRect(x0,0,PANEL_W,BG_H);
