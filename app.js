...
async function loadSvg(){
  gunBox.innerHTML="";
  gunBox.style.backgroundImage=`url('${TEXTURES[viewIdx]}')`;
  const svgText=await fetch(SVG_FILES[viewIdx]).then(r=>r.text());
  gunBox.insertAdjacentHTML("beforeend",svgText);
  ...
  // załaduj zapisane kolory (przy przełączaniu widoku)
  Object.entries(selections).forEach(([id,code])=>{
    const hex=COLORS[Object.keys(COLORS).find(k=>k.startsWith(code))];
    if(hex) applyColor(id,hex,code,true);
  });
}
...
function buildUI(){
  ...
  ["left","right"].forEach(dir=>{
    ...
    a.className=`arrow arrow-${dir}`;
    a.innerHTML=`<svg viewBox="0 0 24 24"><path d="M${dir==="left"?"15 18l-6-6 6-6":"9 6l6 6-6 6"}"/></svg>`;
    a.style.cssText=`
      position:absolute;top:50%;${dir}:4px;transform:translateY(-50%);
      width:40px;height:80px;display:flex;align-items:center;justify-content:center;
      cursor:pointer;opacity:.3;transition:.2s`;
    a.onmouseenter=()=>a.style.opacity=.9;
    a.onmouseleave=()=>a.style.opacity=.3;
    a.onclick=()=>{viewIdx=1-viewIdx;loadSvg();};
    gunBox.appendChild(a);
  });
...
/* apply colour */
function applyColor(id,hex,code,silent=false){
  if(!id){if(!silent) alert(lang==="pl"?"Najpierw wybierz część":"Select a part first");return;}
  ["1","2"].forEach(n=>{
    const ov=document.getElementById(`color-overlay-${n}-${id}`);
    if(ov)(ov.tagName==="g"?ov.querySelectorAll("*"):[ov]).forEach(s=>s.style.fill=hex);
  });
  selections[id]=code;if(!silent){updateSummary();updatePrice();}
}
...
async function savePng(){
  ...
  ctx.drawImage(await loadImg(TEXTURES[viewIdx]),0,0,1600,1200);
  const svg=gunBox.querySelector("svg");
