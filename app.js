/**
 * Główny skrypt aplikacji konfiguratora broni Weapon-Wizards.
 * Zarządza stanem aplikacji, interfejsem użytkownika, logiką kolorowania i generowaniem obrazu.
 * Wersja: KOD 03
 */
document.addEventListener("DOMContentLoaded", () => {

  /* === KONFIGURACJA === */
  /** Ścieżka do pliku SVG z modelem broni. */
  const SVG_FILE = "g17.svg";
  /** Ścieżka do tekstury broni (nakładka). */
  const TEXTURE = "img/glock17.png";
  /** Lista ścieżek do obrazów tła. */
  const BG = ["img/t1.png", "img/t2.png", "img/t3.png", "img/t4.png", "img/t5.png", "img/t6.png", "img/t7.png"];
  /** Cennik dla poszczególnych części. */
  const PRICE = { zamek: 400, szkielet: 400, spust: 150, lufa: 200, zerdz: 50, pazur: 50, zrzut: 50, blokadap: 50, blokada2: 50, pin: 50, stopka: 150 }; // płytka = 0
  /** Progi cenowe dla malowania w systemie MIX. */
  const MIX2 = 800, MIXN = 1000;

  /* === SELEKTORY DOM === */
  /**
   * Skrócona funkcja do pobierania elementów DOM po ID.
   * @param {string} id - ID elementu do znalezienia.
   * @returns {HTMLElement} Znaleziony element.
   */
  ...
  /** @type {Array<Object>} Lista części broni, ładowana z parts.json. */
  let PARTS = [];
  /** @type {Object} Paleta kolorów, ładowana z colors.json. */
  let COLORS = {};

  /* === STAN APLIKACJI === */
  /** @type {string} Aktualnie wybrany język ('pl' lub 'en'). */
  let lang = "pl";
  /** @type {Object} Przechowuje wybory kolorów dla poszczególnych części (id: kod_koloru). */
  let selections = {};
  /** @type {string|null} ID aktualnie wybranej części do kolorowania. */
  let activePart = null;
  /** @type {number} Indeks aktualnie wyświetlanego tła. */
  let bgIdx = 0;

  /**
   * Główna funkcja inicjalizująca aplikację.
   * Asynchronicznie pobiera dane konfiguracyjne, a następnie buduje i uruchamia interfejs.
   */
  async function initializeApp() {
      try {
          // Krok 1: Pobierz dane konfiguracyjne (części i kolory) z plików JSON.
  ...
  /** Wstępne ładowanie obrazów tła w tle, aby uniknąć migotania. */
  function preloadBGs() { BG.forEach(src => { const i = new Image(); i.src = src; }); }

  /** Ładuje i przetwarza plik SVG, tworząc warstwy do kolorowania. */
  ...
  /**
   * Buduje dynamicznie interfejs użytkownika (przyciski części, paletę kolorów).
   * @param {HTMLElement} partsContainer - Kontener na przyciski części.
   * @param {HTMLElement} paletteContainer - Kontener na paletę kolorów.
   */
  function buildUI(partsContainer, paletteContainer) {
      // Generowanie przycisków części
      PARTS.forEach(p => {
      ...
      // Generowanie przycisków MIX
      ["MIX (≤2)", "MIX (3+)"].forEach((txt, i) => {
          const m = document.createElement("button");
          m.className = "mix"; m.textContent = txt;
          m.onclick = () => mix(i ? undefined : 2);
          partsContainer.appendChild(m);
      });
  ...
  /** Ustawia globalne nasłuchiwacze zdarzeń dla przycisków. */
   ...
   * @param {'pl'|'en'} l - Kod języka.
   */
  function setLang(l) {
      lang = l;
      // Tłumaczenie przycisków części
      ...
      // Tłumaczenia pozostałych elementów
  ...
  /**
   * Obsługuje wybór części do pokolorowania.
   * @param {string} id - ID wybranej części.
   */
  function selectPart(id) {
      // Zaznacza przycisk w obu kontenerach (mobilnym i desktopowym)
      document.querySelectorAll(".parts button").forEach(b => {
  ...
  /**
   * Aplikuje wybrany kolor na aktywnej części.
   * @param {string|null} id - ID aktywnej części.
   * @param {string} hex - Kolor w formacie HEX.
   * @param {string} code - Kod koloru (np. H-146).
   */
  ...
  /**
   * Losowo koloruje wszystkie części.
   * @param {number} [maxCols] - Maksymalna liczba unikalnych kolorów do użycia.
   */
  ...
  /** Resetuje wszystkie wybory kolorów. */
  function resetAll() {
  ...
  /** Aktualizuje i wyświetla szacowany koszt malowania. */
  ...
  /**
   * Tworzy i inicjuje pobieranie obrazu PNG z aktualną konfiguracją.
   * @returns {Promise<string>} URL obrazu w formacie Data URL.
   */
  ...
  /** Przygotowuje i otwiera klienta poczty e-mail z danymi projektu. */
