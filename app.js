...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const PISTOL_SRC_W=843, BG_SRC_W=1600;
    const RATIO=PISTOL_SRC_W/BG_SRC_W;            // 0.5269

    /* CSS zmienne */
    const CSS=()=>getComputedStyle(document.documentElement);
    const gunScale=()=>parseFloat(CSS().getPropertyValue('--gun-scale'));
    const bgScale =()=>parseFloat(CSS().getPropertyValue('--bg-scale'));
    ...
    /* init */
    await loadSvg(); buildUI(); defaultBlack(); changeBackground(); updateText(); updateSummary();
    ...
    function updateSummary(){sumList.innerHTML='';PARTS.forEach(p=>{const c=selections[p.id];if(c){const d=document.createElement('div');d.textContent=`${p[lang]} – ${codeOnly(c)}`;sumList.appendChild(d);}});}
    ...
    async function savePng(){
        ...
        const box=svg.getBBox(), scale=2;
        const pistolW=box.width*scale*gunScale(), pistolH=box.height*scale*gunScale();
